...
AffichageImage::AffichageImage(Note *n,QWidget *parent) : AffichageNote(n,parent)
{
    ...
    labelPath = new QLabel("Chemin d'accès :",this);
    ...
    layoutPrincipal->insertLayout(4,layoutDescription);
    layoutPrincipal->insertLayout(5,layoutImage);
...
void AffichageImage::showImage(Multimedia *img)
{
    QPixmap image(img->getFileUrlNotConst());
...
void AffichageImage::chargerVersion(unsigned int i)
{
    // A ce moment là, la note est obligatoirement un Multimedia
    Multimedia *img = dynamic_cast<Multimedia*>(note->getVersion(i));

    // Test au cas où le dynamique cast échoue
    if(img == nullptr) throw NoteException("La note passé ne correspond pas à une tache, erreur de dynamique cast");

    description->setText(img->getDescription());
    path->setText(img->getFileUrl());
    showImage(img);
